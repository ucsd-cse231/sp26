...
#show: doc => template(
  title: "CSE 231: Boa (Let-bindings and Scope)",
  author: "Ranjit Jhala",
  date: "April 2, 2026",
  doc,
)
...
This will let us learn about

- *Variable scoping* with lexical scope
- *Errors* for duplicate / unbound variables

#v(1em)

== Three Pieces of a Language

Recall! Three pieces define any language we compile:
...
#margin-note[
  *s-expression* parenthesized prefix notation.

  - `<number>`s are base 10 and must be representable as an `i32`
  - `<ident>`s are alphanum (+ hyphens/underscores); starting with a letter
  - `let` has parens around the bind
]
...
enum Expr {
    ...
    Add1(Box<Expr>),
    Sub1(Box<Expr>),
    // Fill in the cases for def-var, use-var
...
== QUIZ: How many variables do we need to *store* ?
